...
describe("menu: add", () => {
  ...
  it("asks for title and four macros, in order", async () => {
    ...
    const asked = mocks.text.mock.calls.map(
      ([opts]) => (opts as { message: string }).message,
    );
...
describe("menu: add validation", () => {
  ...
  const validatorFor = (message: string) => {
    const call = mocks.text.mock.calls.find(
      ([opts]) => (opts as { message: string }).message === message,
    );
    if (!call) throw new Error(`no prompt asked "${message}"`);
    return (call[0] as { validate: (v: string) => string | undefined })
      .validate;
  };
...
describe("menu: edit", () => {
  ...
  it("pre-fills each prompt with the current value", async () => {
    await seed({
      meals: [meal({ id: 1, title: "ground beef", protein: 14 })],
      nextId: 2,
    });
    queueSelect("edit", 1, "exit");
    queueText("", "", "", "", "");
    ...
    const prompts = mocks.text.mock.calls.map(
      ([opts]) => opts as { message: string; placeholder?: string },
    );

    expect(prompts.find((p) => p.message === "Title")!.placeholder).toBe(
      "ground beef",
    );
    expect(prompts.find((p) => p.message === "Protein (g)")!.placeholder).toBe(
      "14",
    );
  });
...
describe("menu: picker hints", () => {
  ...
  const hintFor = (callIndex: number) =>
    (mocks.select.mock.calls[callIndex][0] as { options: { hint?: string }[] })
      .options[0].hint!;

  it("shows every macro under a meal in the delete picker", async () => {
    await seed({
      meals: [meal({ id: 1, cals: 200, protein: 14, carbs: 20, fats: 6 })],
      nextId: 2,
    });
    queueSelect("delete", 1, "exit");
    queueConfirm(false);
