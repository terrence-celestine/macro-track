...
describe("repeat and favorites", () => {
    ...
    it("accepts a shorter name via --as", async () => {
        ...
        expect((await readData()).favorites[0]).toMatchObject({
            name: "beef", protein: 14, carbs: 20, fats: 6, cals: 200,
        })
    })
    ...
    it("keeps working after the source meal is deleted", async () => {
        ...
        expect(code).toBe(0)
        expect((await readData()).meals[0]).toMatchObject({ protein: 14, cals: 200 })
    })
...
describe("clear", () => {
    it("removes all meals and resets the counter", async () => {
        ...
        expect(code).toBe(0)
        expect(await readData()).toEqual({ meals: [], nextId: 1, goals: {}, days: [], favorites: [] })
    })
