...
describe("mealsOn", () => {
  ...
  it("keeps every meal from a day, not just the first", async () => {
    ...
    expect((await mealsOn("2026-07-19")).map((m) => m.title)).toEqual([
      "a",
      "b",
    ]);
  });
...
describe("formatTotals", () => {
  ...
  it("shows every macro and the calorie count", async () => {
    ...
    const text = formatTotals(
      { protein: 18, carbs: 65, fats: 7, cals: 400 },
      2,
    ).join("\n");
  ...
  it("trims floating-point noise", async () => {
    ...
    const text = formatTotals(
      { protein: 0.1 + 0.2, carbs: 0, fats: 0, cals: 0 },
      1,
    ).join("\n");
  ...
  it("keeps whole numbers whole", async () => {
    ...
    const text = formatTotals(
      { protein: 14, carbs: 0, fats: 0, cals: 0 },
      1,
    ).join("\n");
