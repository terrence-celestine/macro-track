...
describe("editMeal", () => {
  ...
  it("keeps createdAt", async () => {
    ...
    expect((await readAll()).meals[0].createdAt).toBe(
      "2026-07-19T14:32:05.123Z",
    );
  });
  ...
  it("reports a missing id", async () => {
    ...
    expect(await editMeal(99, { protein: 40 })).toEqual({
      ok: false,
      reason: "not-found",
    });
  });
  ...
  it("refuses a meal whose day is recorded", async () => {
    ...
    expect(await editMeal(1, { protein: 40 })).toEqual({
      ok: false,
      reason: "day-closed",
    });
  });
