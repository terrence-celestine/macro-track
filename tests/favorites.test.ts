...
describe("addFavorite", () => {
  ...
  it("copies every macro", async () => {
    await seed({
      meals: [meal({ protein: 40, carbs: 3, fats: 5, cals: 220 })],
      nextId: 2,
    });
    const { addFavorite } = await load();
  ...
  it("stores exactly the favorite's own fields, nothing from the meal", async () => {
    await seed({
      meals: [meal({ id: 7, localDate: "2026-07-19" })],
      nextId: 8,
    });
    const { addFavorite } = await load();
  ...
  it("gives the favorite its own createdAt, not the meal's", async () => {
    await seed({
      meals: [meal({ createdAt: "2020-01-01T00:00:00.000Z" })],
      nextId: 2,
    });
    const { addFavorite } = await load();
    ...
    expect((await readAll()).favorites[0].createdAt).not.toBe(
  ...
  it("refuses a name already in use", async () => {
    await seed({
      meals: [meal()],
      nextId: 2,
      favorites: [favorite({ name: "beef" })],
    });
    const { addFavorite } = await load();

    expect(await addFavorite(1, "beef")).toEqual({
      ok: false,
  ...
  it("refuses an empty name", async () => {
    ...
    expect(await addFavorite(1, "   ")).toEqual({
      ok: false,
      reason: "empty-name",
    });
  });
...
describe("repeatFavorite", () => {
  ...
  it("copies every macro onto the new meal", async () => {
    await seed({
      favorites: [favorite({ protein: 40, carbs: 3, fats: 5, cals: 220 })],
    });
    const { repeatFavorite } = await load();
  ...
  it("reports an unknown name", async () => {
    ...
    expect(await repeatFavorite("chicken")).toEqual({
      ok: false,
      reason: "not-found",
    });
  });
  ...
  it("counts toward today's totals", async () => {
    ...
    expect(sumMacros(await todaysMeals())).toEqual({
      protein: 28,
      carbs: 40,
      fats: 12,
      cals: 400,
...
describe("favorite output", () => {
  ...
  it("lists each name with its macros", async () => {
    ...
    const text = formatFavorites([
      favorite({ name: "beef", cals: 200, protein: 14 }),
    ]).join("\n");
  ...
  it("tells you how to log it after saving", async () => {
    ...
    expect(formatFavorited(favorite({ name: "beef" }))).toContain(
      "repeat beef",
    );
  });
  ...
  it("suggests --as on a duplicate name", async () => {
    ...
    expect(formatFavoriteFailure("duplicate-name", 1, "beef")).toContain(
      "--as",
    );
  });
