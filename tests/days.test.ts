...
describe("dayHit", () => {
  ...
  it("is true when every set macro hits", async () => {
    ...
    expect(
      dayHit(macros({ protein: 200, cals: 1800 }), {
        protein: 180,
        cals: 2000,
      }),
    ).toBe(true);
  ...
  it("is false when any set macro misses", async () => {
    ...
    expect(
      dayHit(macros({ protein: 200, cals: 2400 }), {
        protein: 180,
        cals: 2000,
      }),
    ).toBe(false);
  ...
  it("ignores macros with no target", async () => {
    ...
    // against the day.
    expect(
      dayHit(macros({ protein: 200, carbs: 9999 }), { protein: 180 }),
    ).toBe(true);
  });
...
describe("buildDayRecord", () => {
  ...
  it("records how many meals it summed", async () => {
    ...
    expect(
      buildDayRecord("2026-07-19", [meal(), meal()], goals).mealCount,
    ).toBe(2);
  });
...
describe("closeStaleDays", () => {
  ...
  it("closes several stale days at once, oldest first", async () => {
    ...
    expect(created.map((d) => d.date)).toEqual([
      "2026-07-16",
      "2026-07-17",
      "2026-07-18",
    ]);
...
describe("getHistory", () => {
  ...
  it("returns most recent first", async () => {
    await seed({
      days: [day("2026-07-16"), day("2026-07-18"), day("2026-07-17")],
    });
    const { getHistory } = await load();

    expect((await getHistory()).map((d) => d.date)).toEqual([
      "2026-07-18",
      "2026-07-17",
      "2026-07-16",
    ]);
  ...
  it("honours a limit", async () => {
    await seed({
      days: [day("2026-07-16"), day("2026-07-17"), day("2026-07-18")],
    });
    const { getHistory } = await load();

    expect((await getHistory(2)).map((d) => d.date)).toEqual([
      "2026-07-18",
      "2026-07-17",
    ]);
  });
  ...
  it("does not mutate the stored order", async () => {
    ...
    expect((await readAll()).days.map((d) => d.date)).toEqual([
      "2026-07-16",
      "2026-07-18",
    ]);
  });
