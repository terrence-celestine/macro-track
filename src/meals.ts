...
export type MealResult =
  { ok: true; meal: Meal } | { ok: false; reason: MealFailure };
 ...
 */
export type MealEdit = Partial<
  Pick<Meal, "title" | "protein" | "carbs" | "fats" | "cals">
>;
...
export const todaysMeals = async (): Promise<Meal[]> =>
  mealsOn(todayLocalDate());

/**
 * Empties the meal log and restarts numbering.
 *
 * Clears meals and nothing else. It used to rebuild from defaultData() and copy
 * a few fields back, which meant every new top-level key silently became
 * something `clear` destroyed — that quietly took day records and favourites
 * with it. Naming what changes rather than what survives makes the next key
 * safe by default.
 *
 * Frozen day records in particular cannot be rebuilt from anything, so losing
 * them to a command documented as clearing meals was pure data loss.
 */
export const clearMeals = async (): Promise<void> => {
  const data = await readData();

  await writeData({ ...data, meals: [], nextId: 1 });
};
