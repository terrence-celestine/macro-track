...
program
  .name("macro-track")
  .description("Track my macros and meals for the day")
  .version("0.1.0");
...
program
  ...
  .action(
    async (
      title: string,
      options: { protein: number; fats: number; carbs: number; kcals: number },
    ) => {
      const REQUIRED_OPTS = ["protein", "fats", "carbs", "kcals"] as const;
      const missingOptions = REQUIRED_OPTS.filter(
...
program
  ...
  .argument(
    "<weight>",
    "your weight, in whatever unit your scale reads",
    parseGrams,
  )
  .action(async (weight: number) => {
    // One reading per day: a second weigh-in replaces the first, since it's
    // a correction rather than a second data point.
...
weight
  ...
  .action(async (options: { days: number }) => {
    ...
    for (const line of formatWeights(
      weights,
...
const goal = program.command("goal").description("Daily macro targets");
  ...
  .action(
    async (options: {
      ...
      if (Object.values(update).every((value) => value === undefined)) {
        fail(
          chalk.yellowBright(
            `Set at least one of --protein, --carbs, --fats, --kcals`,
          ),
        );
      }
...
program
  ...
  .action(async () => {
    ...
    for (const line of formatTotals(
      sumMacros(meals),
      meals.length,
      await getGoals(),
    )) {
...
program
  ...
  .action(async (options: { all?: boolean }) => {
    ...
    if (meals.length === 0) console.log(chalk.red(`You have no meals to show`));
...
program
  ...
  .action(async (name: string) => {
    ...
    if (!result.ok) {
      fail(
        chalk.red(
          `No favorite called "${name}". Run \`favorite list\` to see them.`,
        ),
      );
    }
...
const favorite = program.command("favorite").description("Meals you log often");
  ...
  .action(
    async (
      ...
      if (Object.values(update).every((value) => value === undefined)) {
        fail(
          chalk.yellowBright(
            `Set at least one of --title, --protein, --carbs, --fats, --kcals`,
          ),
        );
      }
